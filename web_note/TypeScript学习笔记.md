...
> TypeScript 会在**没有明确的指定类型的时候推测出一个类型**，这就是类型推论。
>
...
//(x:number,y:number,z?:number)=>number代表的是函数类型
...
知道了它们的核心区别，就知道了**类型声明是比类型断言**更加严格的。
...
而当我们使用 `const` 定义时，表示**此时的全局变量是一个常量**，不允许再去修改它的值了：
...
`namespace` 是 ts 早期时为了解决模块化而创造的关键字，中文称为**命名空间**。
...
3. interface的继承使用extends关键字，type使用&
4. 可以用type给类型起别名，也可以用type约束字符串字面量类型，用来约束取值只能是某几个字符串中的一个
...
常数枚举与普通枚举的区别是，它会在**编译阶段被删除**，并且不能包含计算成员。
...
TypeScript 可以使用**三种访问修饰符**（Access Modifiers），分别是 `public`、`private` 和 `protected`。
